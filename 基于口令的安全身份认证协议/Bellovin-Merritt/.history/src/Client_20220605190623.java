...
public class Client extends Thread {
    ...
    public void run() {
        try {
            ...
            // Generate N_A
            byte[] N_A = rsa.genRandomString(rsa.N_len).getBytes();
            ...
            if (Arrays.equals(N_1, N_A)) {
                ...
                byte[] N_2 = new byte[rsa.N_len];
                System.arraycopy(Concat, rsa.N_len, N_2, 0, rsa.N_len);
                // Send 3: E_1(K_s, N_2)
            ...
            String original_msg = "RNG wins the MSI 2022!";
            byte[] msg = original_msg.getBytes("utf-8");
            // Send 4: E_1(K_s, msg)
            out.writeObject(des.encrypt(msg));
            out.flush();
            System.out.println("[Client] Message sent!");
            // Receive 4: E_1(K_s, message)
            byte[] msg_cipher = (byte[]) in.readObject();
            System.out.println("[Client] Read Message from server!");
            // decrypt message_cipher to get message
            byte[] msg_decrypted = des.decrypt(msg_cipher);
            System.out.println("[Client] <Server> " + new String(msg_decrypted));
